...
@Component({
  selector: 'angualert',
  templateUrl: './angualert.component.html',
  styleUrls: ['./angualert.component.scss'],
  animations: [
    trigger('enterLeave', [
...
export class AngualertComponent {
  @Input() alertType: string = 'info';
  @Output() alertConfirmed: EventEmitter<boolean> = null;
  @Input() alert: Alert;
  public onDestroy$: EventEmitter<AngualertComponent>; // Reference to itself, so it can be destroyed
  ...
  constructor() {
    this.onDestroy$ = new EventEmitter<AngualertComponent>();
    this.progress = 0;
    this.currentTime = 0;
